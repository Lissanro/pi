...
- `/continue` now continues an assistant message via prefill continuation for openai-completions providers (e.g., llama-server). The last assistant message is removed from the transcript and re-sent with `return_prefill` so the model echoes it back with newly generated tokens, replacing the original. Assistant messages with tool calls are now supported: the raw tool-call tokens are sent via `tool_calls_raw` so the provider can resume both complete and partial tool calls, and the echoed tool calls are verified against the prefill (mismatch restores the original partial).
...
- Added direct message selection to `/fork` and extended forking beyond user messages: `/fork N` forks from the message at index N (0 = latest non-harness message, counting backwards over user and assistant messages) and `/fork <substring>` (quoted, plain, or pasted below the command) forks from the single message whose search haystack contains it. Forking from a user message cuts before it and loads its text into the editor; forking from an assistant message keeps it as the branch leaf so the session can continue from that point. The bare `/fork` selector now lists all non-harness messages. Out-of-bounds indices and absent or non-unique substring matches are reported without forking.
- `/edit`, `/delete`, and `/fork` now accept the substring below the command line as well as on the same line, so a pasted message (or a large part of it, including `/copy`/`/edit`-format output with thinking and tool calls) can be used as the search pattern.
- Added `/schedule` to queue a message (written below the command) for later delivery. `/schedule` with no parameters sends it when the agent finishes the current task (unlike steering messages, which are delivered after each assistant message); `/schedule 3` sends after 3 more user/assistant messages (harness messages are not counted); `/schedule 1h`, `/schedule 5.5m`, or summed sleep(1)-style durations (`/schedule 1h 30m`, units s/m/h/d) send after a relative delay; and `/schedule 2026-08-08 00:34` or `/schedule 2026-08-09 10:14:59` send at an absolute local time. Messages that fire while the agent is streaming or compacting are queued as follow-ups; pending scheduled messages are cleared when the session is replaced.
...
- Fixed context size / token limit errors ("context size has been exceeded", "context window limit", etc.) incorrectly triggering compaction retries. These are now classified as non-retryable errors since retrying won't help when the context window is already full.
...
- Fixed `/continue` issued while `/compact` is running racing the compaction and corrupting session state (stuck streaming, partial messages dropped). A `/continue` typed during compaction is now queued and runs once the session is idle after compaction completes; direct callers are rejected with a clear error while compaction is in flight.
- Fixed a `/continue` after a completed `/compact` losing the compaction entry: the prefill capture deleted the trailing compaction entry, overflowing context on the next continue, and computed the wrong restore point so a failed continuation threw "Entry not found". The compaction entry now survives both successful and failed prefill continuations.
