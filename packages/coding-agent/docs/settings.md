...
| `retry.maxRetries` | number | `Infinity` | Maximum agent-level retry attempts (unlimited by default) |
| `retry.baseDelayMs` | number | `2000` | Base delay for agent-level exponential backoff (2s, 4s, 8s, ...) |
| `retry.maxBackoffMs` | number | `300000` | Cap for agent-level exponential backoff delay (5 minutes) |
...
By default, agent-level retry retries indefinitely with exponential backoff capped at `retry.maxBackoffMs` (5 minutes). This lets the agent recover automatically from transient backend outages. Set `retry.maxRetries` to a finite number to limit attempts.
